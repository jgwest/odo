...
// CreateConsoleOutputWriterAndChannel is a utility function that returns a pipeWriter and a channel; any strings written
// to that PipeWriter will be output to the channel (as lines) when the writer closes. This is used to retrieve the
// stdout/stderr output from the container exec commands.
//
// The io.PipeWriter can be passed to ExecuteCommand(...) above, in order to receive the full stderr/stdout output from the process.
